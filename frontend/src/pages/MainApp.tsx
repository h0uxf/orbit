...
import { Stars, StatusBar } from '../components/atoms';
      ...
      <div className="phone">
        <div className="screen">
          ...
          <SettingsSheet
            ...
            onResetOnboarding={() => {
              o.setSettingsOpen(false);
              // Persist first, then full-reload so the parent route re-checks
              // auth+onboarded fresh from the backend (it only reads this once on mount).
              api.patch('/api/state', { onboarded: false }).finally(() => { window.location.href = '/'; });
            }}
            onOpenAccount={() => { o.setSettingsOpen(false); navigate('/account'); }}
          />
